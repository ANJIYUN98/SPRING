...
import com.example.app.listener.RequestHandlerEventListener;
...
public class WebMvcConfig implements WebMvcConfigurer{
	...
	public MultipartResolver multipartResolver() {
		...
		multipartResolver.setMaxUploadSize(20971520); 			//20MB	//전체 업로드 허용 사이즈 //1024*1024*10*2 = 20971520
		multipartResolver.setMaxUploadSizePerFile(20971520); 	//20MB	//파일 1개당 허용 가능한 업로드
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		registry.addResourceHandler("/resources/**").addResourceLocations("/resources/");  //css, js 포함
	}
	...
	//LISTENER Bean 등록
	@Bean
	public RequestHandlerEventListener requestHandlerEventListener() {
		return new RequestHandlerEventListener();
	}
	...
	//02 SimpleUrlHandlerMapping : 개발자가 직접 매핑정보를 설정, 정적 자원에 대한 매핑정보 설정이 기본값으로 설정
	@Bean
	...
	//03 RequestMappingHandlerMapping : Controller와 매핑되는 URL을 찾아내고 해당 URL에 대한 요청을 처리함
	//적적한 컨트롤러 및 메서드를 찾아 매핑( @RequestMapping, @GetMapping, @PostMapping... )
	@Bean
	RequestMappingHandlerMapping requestMappingHandlerMapping() throws Exception {
		...
		//요청 매핑정보, 핸들러, 메서드 등록(handlerMapping에 위에 적은 것들을 넣어주는 작업)
