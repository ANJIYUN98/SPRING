...
<h1>REST TEST PAGE</h1>
	...
	<script>
		...
		const projectPath='${pageContext.request.contextPath}'; //project 경로
	
		//비동기 GET MEMO ADD
		const axiosAsyncGetBtn = document.querySelector('.axiosAsyncGetBtn'); //axiosAsyncGetBtn는 위에서 작성한 [ADD]비동기 GET 요청 버튼
		axiosAsyncGetBtn.addEventListener('click',function(){
			
			const axiosAsyncGetForm = document.axiosAsyncGetForm; //axiosAsyncGetForm는 [ADD]비동기 GET 요청 form 이름
			...
			axios.get(projectPath+"/memo/add_get?id="+id+"&text="+text)
				...
				.catch(err=>{console.log(err);})	//요청 실패했을 때
		...
		axiosAsyncPostBtn.addEventListener("click",function(){
			...
			const header={"Content-Type":"application/json"} //json형태
			
			const params={"id":id,"text":text} //const로 받은 id와 text를 넣어줌
